...
function randomQuestion() {
  ...
  const validQuesLength = valid_questions.length

  /*check if all questions have been gone through*/
  if (validQuesLength === used_questions.length) {
    used_questions = [];
  }

  let rnd = Math.floor(Math.random() * len);

  while (used_questions.indexOf(rnd) != -1 || valid_questions.indexOf(rnd) === -1) {
  ...
  document.getElementById('myNum').value = len;

  used_questions.push(rnd);

  numer();
}

function numer() {
  document.getElementById('numer').innerHTML = used_questions.length;
}
...
const questions = [
  ...
  { question: "What is a linked list?", answer: "A linked list is a linear data structure in which the elements are NOT necessarily stored in a contiguous manner. The data elements are instead linked via 'pointers' which index the location of the following element in the sequence." },
  ...
  { question: "What's the difference between stacks and arrays?", answer: "A stack follows the last in, first out principle (LIFO) and so to access any of the data, you must start at the top of the stack. Arrays do not follow any specific order. Instead, data can be accessed by refering to the index of the element." },
  
  { question: "What are classes and objects? (OOP)", answer: "A class is a collection of objects which the user defines. Classes are types of data structures that make the data easier to sort, find, manipulate, etc. Objects are instances of a class created with specifically defined data." },
  { question: "What are class constructors? (OOP)", answer: "Class constructors assign values to the data members of the class when an object of the class is created. For more information on OOP, as well as to find examples, visit: https://www.analyticsvidhya.com/blog/2020/09/object-oriented-programming/#:~:text=Now%2C%20there%20are%20four%20fundamental,in%20order%20to%20understand%20OOPs." },
  ...
  { question: "What is abstraction? (OOP)", answer: "Abstraction is a concept where only necessary attributes of a larger data pool are shown to the user. Unnecessary data is hidden from the users to reduce complexity." },
  { question: "What is polymorphism? (OOP)", answer: "Polymorphism describes situations in which something occurs in several different forms. It describes the concept that you can access objects of different types through the same interface and each type can provide its own independent implementation of this interface." },
  { question: "What is inheritance? (OOP)", answer: "Inheritance is a concept in which the 'child' class will inherit all the public and protected properties and methods/behaviors from the 'parent' class. In addition, it can have its own properties and methods." },
  { question: "What is encapsulation? (OOP)", answer: "Encapsulation refers to the bunching of data and code into a single unit. A common example of encapsulation are classes in OOP (object-oriented programming) languages. Classes contain both variables (data) and behaviours (functions/methods)." },
];
...
/*had to put the window onload as otherwise the console throws an error as it executes before page is loaded*/
...
function startGame() {
  /*should first check which boxes are checked and set up the valid list of quesitons this way*/
  if (document.getElementById('one').checked) {
    for (let i = oopStart; i <= oopEnd; i++) {
      valid_questions.push(i);
    }
  }

  if (document.getElementById('two').checked) {
    for (let i = generalStart; i <= generalEnd; i++) {
      valid_questions.push(i);
    }
  }
...
function fraction() {
  document.getElementById('denom').innerHTML = valid_questions.length;
  document.getElementById('numer').innerHTML = used_questions.length;
}